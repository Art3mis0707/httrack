...
html_save_dir = 'C:\\Images\\HTML'  # Directory to save HTML code
...
def save_image_html_code(element, index, file_path):
    html_code = element.get_attribute('outerHTML')
    with open(file_path, 'w', encoding='utf-8') as file:
        file.write(html_code)
    print(f'HTML code saved: {file_path}')
        ...
        for file in files:
            if file.endswith('.html'):
                ...
                for index, image in enumerate(images):
                    ...
                    html_save_path = os.path.join(html_save_dir, f'{os.path.splitext(file)[0]}_image_{index}.txt')
                    capture_element_screenshot(driver, image, image_save_path)
                    save_image_html_code(image, index, html_save_path)
